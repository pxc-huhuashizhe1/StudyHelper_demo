...
static void HandDetectFlag(const RecogNumInfo resBuf)
{
    ...
    switch (resBuf.num) {
        case 0u:
            ...
            UartSend(uartFd,(unsigned char *)gestureName,strlen(gestureName));
        ...
        default:
            ...
            UartSend(uartFd,(unsigned char *)gestureName,strlen(gestureName));
            unsigned int RecvLen3 = strlen(gestureName);
            readLen = UartRead(uartFd, uartReadBuff, RecvLen3, 1000);
            SAMPLE_PRT("----gesture name----:%s\n", uartReadBuff);
            // SAMPLE_PRT("----len1:%d len2:%d----\n", RecvLen3,strlen(gestureName));
            break;
    }
